"""
3DES (Triple DES) Encryption/Decryption Performance Analysis
...
def generate_3des_key(student_id):
    """
    3DES key = student ID repeated/padded to 24 bytes
    Key structure: K1 (8 bytes) + K2 (8 bytes) + K3 (8 bytes)
    """
    base = student_id * 3
    key = base[:24].encode('utf-8')
    return key


def perform_3des_ecb(data, key):
...
def main():
    print("=" * 70)
    print("3DES Encryption/Decryption Performance Analysis (ECB vs CBC)")
    print("=" * 70)

    # Generate and display key
    des3_key = generate_3des_key(STUDENT_ID)
    print("\nEncryption Details:")
    print("-" * 50)
    print(f"Student ID: {STUDENT_ID}")
    print(f"Algorithm: 3DES (Triple Data Encryption Standard)")
    print(f"Key Size: 192-bit (24 bytes)")
    print(f"3DES Key (hex): {des3_key.hex()}")
    print(f"Block Size: 8 bytes")
    print(f"Key Structure: K1 + K2 + K3 (each 8 bytes)")
    ...
    # ECB Mode
    print("\n[ECB Mode]")
    encrypted_ecb, decrypted_ecb, enc_time_ecb, dec_time_ecb = perform_3des_ecb(plaintext, des3_key)
    ...
    # CBC Mode
    print("\n[CBC Mode]")
    encrypted_cbc, decrypted_cbc, enc_time_cbc, dec_time_cbc, iv = perform_3des_cbc(plaintext, des3_key)
