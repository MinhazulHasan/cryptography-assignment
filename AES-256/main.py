"""
AES-256 Encryption/Decryption Performance Analysis
...
def generate_aes256_key(student_id):
    # AES-256 key = student ID padded with '0' to 32 bytes
    key = student_id.ljust(32, '0').encode('utf-8')
    return key


def perform_aes256_ecb(data, key):
...
def perform_aes256_cbc(data, key):
...
def main():
    print("=" * 70)
    print("AES-256 Encryption/Decryption Performance Analysis (ECB vs CBC)")
    print("=" * 70)

    # Generate and display key
    aes_key = generate_aes256_key(STUDENT_ID)
    print("\nEncryption Details:")
    print("-" * 50)
    print(f"Student ID: {STUDENT_ID}")
    print(f"Algorithm: AES (Advanced Encryption Standard)")
    print(f"Key Size: 256-bit (32 bytes)")
    ...
    # ECB Mode
    print("\n[ECB Mode]")
    encrypted_ecb, decrypted_ecb, enc_time_ecb, dec_time_ecb = perform_aes256_ecb(plaintext, aes_key)
    ...
    # CBC Mode
    print("\n[CBC Mode]")
    encrypted_cbc, decrypted_cbc, enc_time_cbc, dec_time_cbc, iv = perform_aes256_cbc(plaintext, aes_key)
